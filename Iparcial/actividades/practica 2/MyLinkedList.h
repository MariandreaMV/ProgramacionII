...
public:
  ...
  void OrdenarStrings(t element){//solo recibe strings
  	
	node<t>* newNode = new node<t>(element);
  if (!head) {
    head = newNode;
  }
  else if (strcmp(head->getInfo().c_str() , element.c_str())>0 ){
    ...
    node<t> *previous, *p;
    previous = p = head;
    while (p->getnext() != NULL && (strcmp(element.c_str() , p->getInfo().c_str() )>0)){
